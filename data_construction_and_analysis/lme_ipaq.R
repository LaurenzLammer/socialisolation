...
path = "/data/pt_life/ResearchProjects/LLammer/si_update/Results_ipaq"
...
fastlme <- function(poi = "LSNS_base", dv, model = 1){
  ...
  if(dv == "HCV"){
    datax = subset(data, data$outlier_HCV != 1 & !is.na(data$IPAQ_total_met))
  }
  else if(dv == "memo"){
    datax = subset(data, data$outlier_memo != 1 & !is.na(data$IPAQ_total_met))
  }
  else if(dv == "exfunct"){
    datax = subset(data, data$outlier_exfunct != 1 & !is.na(data$IPAQ_total_met))
  }
  else{
    datax = subset(data, data$outlier_procspeed != 1 & !is.na(data$IPAQ_total_met))
  }
  if(model == 1){
    if(poi == "LSNS_base"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+IPAQ_total_met+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+IPAQ_total_met+sex"), data=datax)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+IPAQ_total_met+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("LSNS_change", "age_base", "age_change", "sex", "subject", "IPAQ_total_met"))
      }
      ...
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+IPAQ_total_met+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("LSNS_base", "age_base", "age_change", "sex", "subject", "IPAQ_total_met"))
      }
    else if(poi == "LSNS_base:age_change"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+LSNS_base:age_change+IPAQ_total_met+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+IPAQ_total_met+sex"), data=datax)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+LSNS_base:age_change+IPAQ_total_met+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("LSNS_change", "age_base", "sex", "subject", "^LSNS_base$", "^age_change$", "IPAQ_total_met"))
    }
    else{
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+LSNS_base:LSNS_change+IPAQ_total_met+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+IPAQ_total_met+sex"), data=datax)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+LSNS_base:LSNS_change+IPAQ_total_met+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("age_base", "age_change", "sex", "subject", "^LSNS_base$", "^LSNS_change$", "IPAQ_total_met"))
    }
  } 
    ...
    if(poi == "LSNS_base"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+IPAQ_total_met+(1|subject)"),
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+IPAQ_total_met+CES.D"), 
                 data=datax, na.action = na.omit)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+IPAQ_total_met+subject")), 
            data=datay, whichRandom = "subject", multicore = T, neverExclude = 
            c("LSNS_change", "age_base", "age_change", "sex", "education", "BMI", "hypertension", "diabetes", "CES.D", "subject", "IPAQ_total_met"))
      }
    else if(poi == "LSNS_change"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+IPAQ_total_met+(1|subject)"), 
             data=datax, REML=F, na.action = na.omit)
      ...
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+IPAQ_total_met+subject")), 
            data=datay, whichRandom = "subject", multicore = T, neverExclude = 
            c("LSNS_base", "age_base", "age_change", "sex", "education", "BMI", "hypertension", "diabetes", "CES.D", "subject", "IPAQ_total_met"))
      }
    else if(poi == "LSNS_base:age_change"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+LSNS_base:age_change+IPAQ_total_met+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+IPAQ_total_met+CES.D"), 
                 data=datax, na.action = na.omit)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+LSNS_base:age_change+IPAQ_total_met+subject")), 
                          data=datay, whichRandom = "subject", multicore = T, 
                          neverExclude = c("LSNS_change", "age_base", "sex", "education", "BMI", "hypertension", 
                                           "diabetes", "CES.D", "subject", "^LSNS_base$", "^age_change$", "IPAQ_total_met"))
    }
    else{
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+LSNS_base:LSNS_change+IPAQ_total_met+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+IPAQ_total_met+CES.D"), 
                 data=datax, na.action = na.omit)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+LSNS_base:LSNS_change+IPAQ_total_met+subject")), 
                          data=datay, whichRandom = "subject", multicore = T, 
                          neverExclude = c("age_base", "age_change", "sex", "education", "BMI", "hypertension", 
                                           "diabetes", "CES.D", "subject", "^LSNS_base$", "^LSNS_change$", "IPAQ_total_met"))
    }
  }
