...
test <- lm(HCV~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic, data=datax)
...
bf <- generalTestBF(HCV~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+ LSNS_base:age_change +subject, 
                    data=datax, whichRandom = "subject", multicore = T,
                    neverExclude = c("LSNS_change", "age_base", "sex", "pandemic","subject", "^LSNS_base$", "^age_change$"))
  ...
  if(model == 1){
    if(poi == "LSNS_base"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic"), data=datax)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("LSNS_change", "age_base", "age_change", "sex", "pandemic", "subject"))
      }
      ...
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("LSNS_base", "age_base", "age_change", "sex", "pandemic", "subject"))
      }
      ...
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic"), data=datax)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+LSNS_base:age_change+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("LSNS_change", "age_base", "sex", "pandemic","subject", "^LSNS_base$", "^age_change$"))
    }
    else{
      ...
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic"), data=datax)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+LSNS_base:LSNS_change+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("age_base", "age_change", "sex", "pandemic", "subject", "^LSNS_base$", "^LSNS_change$"))
    }
  } 
    ...
    if(poi == "LSNS_base"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D+(1|subject)"),
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D"), 
                 data=datax, na.action = na.omit)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D+subject")), 
            data=datay, whichRandom = "subject", multicore = T, neverExclude = 
            c("LSNS_change", "age_base", "age_change", "sex", "pandemic", "education", "BMI", "hypertension", "diabetes", "CES.D", "subject"))
      }
    else if(poi == "LSNS_change"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D+(1|subject)"), 
             data=datax, REML=F, na.action = na.omit)
      ...
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D+subject")), 
            data=datay, whichRandom = "subject", multicore = T, neverExclude = 
            c("LSNS_base", "age_base", "age_change", "sex", "pandemic", "education", "BMI", "hypertension", "diabetes", "CES.D", "subject"))
      }
    else if(poi == "LSNS_base:age_change"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D+LSNS_base:age_change+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D"), 
                 data=datax, na.action = na.omit)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D+LSNS_base:age_change+subject")), 
                          data=datay, whichRandom = "subject", multicore = T, 
                          neverExclude = c("LSNS_change", "age_base", "sex", "pandemic", "education", "BMI", "hypertension", 
                                           "diabetes", "CES.D", "subject", "^LSNS_base$", "^age_change$"))
    }
    else{
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D+LSNS_base:LSNS_change+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D"), 
                 data=datax, na.action = na.omit)
      vifres <- as.data.frame(vif(test))
      bf <- generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+education+BMI+hypertension+diabetes+CES.D+LSNS_base:LSNS_change+subject")), 
                          data=datay, whichRandom = "subject", multicore = T, 
                          neverExclude = c("age_base", "age_change", "sex", "pandemic", "education", "BMI", "hypertension", 
