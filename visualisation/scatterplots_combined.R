load("/data/pt_life/ResearchProjects/LLammer/si_update/Results/Workspace/workspace.RData")
library(effects)
library(ggplot2)
library(cowplot)
library(ggplotify)
...
comp_plot <- function(dv, model = 1){
  ...
  p1 <- ggplot() +
    ...
    geom_line(data = lbe, aes(LSNS_base, fit), col = linecol) +
    geom_ribbon(data = lbe, aes(x=LSNS_base, ymin=lower, ymax=upper), alpha= 0.3, fill = linecol) +
    theme_bw() +
  ...
  p2 <- ggplot() +
    ...
    geom_line(data = lce, aes(LSNS_change, fit), col = linecol) +
    geom_ribbon(data = lce, aes(x=LSNS_change, ymin=lower, ymax=upper), alpha= 0.3, fill = linecol) +
    theme_bw() +
    theme(axis.title.y = element_blank(), axis.text.y = element_blank()) +
    xlim(-17,17) +
...
ggsave(filename = "/data/pt_life/ResearchProjects/LLammer/si_update/Results/visualisation/model1_scatterplots.tff", device = "tiff", dpi = 600, width = 8, height = 12.8)
