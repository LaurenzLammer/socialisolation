...
res <- lmer(HCV~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+LSNS_base:age_change+(1|subject), 
...
write.csv(coeff, file = "/data/pt_life/LLammer/Results/pand/coefficients/HCV_on_LSNS_base:age_change_model1.csv")
# linear (non-mixed) model to calculate VIFs 
test <- lm(HCV~LSNS_base+LSNS_change+age_base+age_change+pandemic+sex, data=datax)
# calculate and save VIFs
vifres <- as.data.frame(vif(test))
write.csv(vifres, file = "/data/pt_life/LLammer/Results/pand/VIFs/HCV_on_LSNS_base:age_change_model1.csv")
...
bf <- extractBF((generalTestBF(HCV~LSNS_base+LSNS_change+age_base+age_change+sex+ pandemic+LSNS_base:age_change +subject, 
                    data=datax, whichRandom = "subject", multicore = T,
                    neverExclude = c("LSNS_change", "age_base", "sex", "subject", "^LSNS_base$", "^age_change$", "pandemic"))),
                logbf = F)
...
png(filename = "/data/pt_life/LLammer/Results/pand/diag_plots/HCV_on_LSNS_base:age_change_model1.png")
...
write.csv(stab_results$summary, file = "/data/pt_life/LLammer/Results/pand/model_stab//HCV_on_LSNS_base:age_change_model1.csv")
...
list151 <- list(res, stab_results, vifres, bf, p)
names(list151) <- c("res", "stab_results", "vifres", "bf", "p")
...
fastlme <- function(poi = "LSNS_base", dv, model = 1){
  ...
  if(model == 1){
    if(poi == "LSNS_base"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+pandemic+sex"), data=datax)
      vifres <- as.data.frame(vif(test))
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("LSNS_change", "age_base", "age_change", "sex", "subject", "pandemic"))),
                      logbf = F)
      bf["full_vs_null", 1] <- bf[1,1] / bf[2,1]
      }
      ...
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("LSNS_base", "age_base", "age_change", "sex", "subject", "pandemic"))),
                      logbf = F)
      bf["full_vs_null", 1] <- bf[1,1] / bf[2,1]
      }
    else if(poi == "LSNS_base:age_change"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+LSNS_base:age_change+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+pandemic+sex"), data=datax)
      vifres <- as.data.frame(vif(test))
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+LSNS_base:age_change+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("LSNS_change", "age_base", "sex", "subject", "^LSNS_base$", "^age_change$", "pandemic"))),
                      logbf = F)
      bf["full_vs_null", 1] <- bf[1,1] / bf[2,1]
      ...
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+LSNS_base:LSNS_change+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+pandemic+sex"), data=datax)
      vifres <- as.data.frame(vif(test))
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+pandemic+LSNS_base:LSNS_change+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("age_base", "age_change", "sex", "subject", "pandemic", "^LSNS_base$", "^LSNS_change$"))),
                      logbf = F)
      bf["full_vs_null", 1] <- bf[1,1] / bf[2,1]
    }
  } 
    ...
    if(poi == "LSNS_base"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+pandemic+(1|subject)"),
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+pandemic"), 
                 data=datax, na.action = na.omit)
      vifres <- as.data.frame(vif(test))
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+pandemic+subject")), 
            data=datay, whichRandom = "subject", multicore = T, neverExclude = 
            c("LSNS_change", "age_base", "age_change", "sex", "education", "BMI", "hypertension", "diabetes", "CES.D", "subject", "pandemic"))),
            logbf = F)
      bf["full_vs_null", 1] <- bf[1,1] / bf[2,1]
      }
    else if(poi == "LSNS_change"){
      ...
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+pandemic+subject")), 
            data=datay, whichRandom = "subject", multicore = T, neverExclude = 
            c("LSNS_base", "age_base", "age_change", "sex", "education", "BMI", "hypertension", "diabetes", "CES.D", "subject", "pandemic"))),
            logbf = F)
      bf["full_vs_null", 1] <- bf[1,1] / bf[2,1]
      }
    else if(poi == "LSNS_base:age_change"){
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+pandemic+LSNS_base:age_change+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+pandemic"), 
                 data=datax, na.action = na.omit)
      vifres <- as.data.frame(vif(test))
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+pandemic+LSNS_base:age_change+subject")), 
                          data=datay, whichRandom = "subject", multicore = T, 
                          neverExclude = c("LSNS_change", "age_base", "sex", "education", "BMI", "hypertension", "pandemic",
                                           "diabetes", "CES.D", "subject", "^LSNS_base$", "^age_change$"))), logbf = F)
      bf["full_vs_null", 1] <- bf[1,1] / bf[2,1]
    }
    else{
      res <- lmer(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+pandemic+LSNS_base:LSNS_change+(1|subject)"), 
                  data=datax, REML=F, na.action = na.omit)
      test <- lm(formula = paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+pandemic"), 
                 data=datax, na.action = na.omit)
      vifres <- as.data.frame(vif(test))
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+pandemic+LSNS_base:LSNS_change+subject")), 
                          data=datay, whichRandom = "subject", multicore = T, 
                          neverExclude = c("age_base", "age_change", "sex", "education", "BMI", "hypertension", "pandemic",
  ...
  write.csv(vifres,
            file = paste0("/data/pt_life/LLammer/Results/pand/VIFs/", dv, "_on_", poi, "_model", model, ".csv"))
  sres <- summary(res)
  coeff <- as.data.frame(sres$coefficients)
  CI <- as.data.frame(confint.merMod(res))
  coeff <- merge(coeff, CI, all = T, by="row.names")
  ...
  coeff$n_individual <- ngrps(res)
  write.csv(coeff, 
            file = paste0("/data/pt_life/LLammer/Results/pand/coefficients/", dv, "_on_", poi, "_model", model, ".csv"))
  png(filename = paste0("/data/pt_life/LLammer/Results/pand/diag_plots/", dv, "_on_", poi, "_model", model, ".png"))
  diagnostics.plot(res)
  dev.off()
  stab_results <- glmm.model.stab(res)
  write.csv(stab_results$summary, 
            file = paste0("/data/pt_life/LLammer/Results/pand/model_stab/", dv, "_on_", poi, "_model", model, ".csv"))
  pval_res <- as_lmerModLmerTest(res)
  p <- as.data.frame(drop1(pval_res, scope = poi, ddf = "Satterthwaite"))
  p$qval <- 0
  p$significance <- "not applicable"
  list <- list(res, stab_results, vifres, bf, p)
  names(list) <- c("res", "stab_results", "vifres", "bf", "p")
  return(list)
}
...
qval1 <- qvalue(c(list111$p$`Pr(>F)`, list131$p$`Pr(>F)`, list151$p$`Pr(>F)`, list211a$p$`Pr(>F)`, list211b$p$`Pr(>F)`, 
                  list211c$p$`Pr(>F)`, list221a$p$`Pr(>F)`, list221b$p$`Pr(>F)`, list221c$p$`Pr(>F)`, list231a$p$`Pr(>F)`,
                  list231b$p$`Pr(>F)`, list231c$p$`Pr(>F)`), fdr.level = 0.05, pi0 = 1)

qval2 <- qvalue(c(list112$p$`Pr(>F)`, list132$p$`Pr(>F)`, list152$p$`Pr(>F)`, list212a$p$`Pr(>F)`, list212b$p$`Pr(>F)`, 
                  list212c$p$`Pr(>F)`, list222a$p$`Pr(>F)`, list222b$p$`Pr(>F)`, list222c$p$`Pr(>F)`, list232a$p$`Pr(>F)`,
                  list232b$p$`Pr(>F)`, list232c$p$`Pr(>F)`), fdr.level = 0.05, pi0 = 1)
...
write.csv(vif_tresh_check, file = "/data/pt_life/LLammer/Results/pand/VIFs/threshold_check.csv")
...
write.csv(stab_warnings, file = "/data/pt_life/LLammer/Results/pand/model_stab/warnings_check.csv")

# write p-value tables
pvals <- data.frame(matrix(unlist(lapply(comprehensive, function(x) print(x$p))), nrow = length(comprehensive), 
                           byrow = TRUE))
colnames(pvals) <- c("Sum Sq", "Mean Sq", "NumDF", "DenDF",  "F value", "Pr(>F)", "qval", "significance")
rownames(pvals) <- hypotheses
write.csv(pvals, file = "/data/pt_life/LLammer/Results/pand/pvals/overview.csv")

# write a dataframe containing all Bayes-Factor information

BF <- data.frame(matrix(nrow = length(comprehensive)*3, ncol = 4))
BF_names <- c()
counter <- 1
for(n in names(comprehensive)){
  BF[((counter):(counter+2)),] <- comprehensive[[n]]$bf
  BF_names <- c(BF_names, rownames(comprehensive[[n]]$bf))
  counter <- counter + 3
}
...
for(n in (1:length(BF$model))){
  BF$hypothesis[n] <- hypotheses[ceiling(n/3)]
}

write.csv(BF, file = "/data/pt_life/LLammer/Results/pand/bayes_factor/bayes_factors.csv")

save.image("/data/pt_life/LLammer/Results/pand/Workspace/workspace2.RData")
