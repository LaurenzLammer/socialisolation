...
write.csv(coeff, file = "/data/pt_life/LLammer/Results/coefficients/HCV_on_LSNS_base:age_change_model1.csv")
...
write.csv(vifres, file = "/data/pt_life/LLammer/Results/VIFs/HCV_on_LSNS_base:age_change_model1.csv")
...
bf <- extractBF((generalTestBF(HCV~LSNS_base+LSNS_change+age_base+age_change+sex+ LSNS_base:age_change +subject, 
                    data=datax, whichRandom = "subject", multicore = T,
                    neverExclude = c("LSNS_change", "age_base", "sex", "subject"))),
                logbf = F)

# calculate the evidence in favour of the full model compared to the null model
bf["full_vs_null", 1] <- bf[4,1] / bf[3,1]
# GeneralTestBF won't let us drop only the interaction term
# hence, we have to delete other models that were calculated
bf <- bf[c(3,4,6),]
...
png(filename = "/data/pt_life/LLammer/Results/diag_plots/HCV_on_LSNS_base:age_change_model1.png")
...
write.csv(stab_results$summary, file = "/data/pt_life/LLammer/Results/model_stab//HCV_on_LSNS_base:age_change_model1.csv")
...
fastlme <- function(poi = "LSNS_base", dv, model = 1){
  ...
  if(model == 1){
    if(poi == "LSNS_base"){
      ...
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+LSNS_base:LSNS_change+subject")), 
                          data=datax, whichRandom = "subject",  multicore = T,
                          neverExclude = c("age_base", "age_change", "sex", "subject"))),
                      logbf = F)
      bf["full_vs_null", 1] <- bf[4,1] / bf[3,1]
      bf <- bf[c(3,4,6),]
      }
    ...
    if(poi == "LSNS_base"){
      ...
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+LSNS_base:age_change+subject")), 
                          ...
                          neverExclude = c("LSNS_change", "age_base", "sex", "education", "BMI", "hypertension", "diabetes", "CES.D", "subject"))),
                      logbf = F)
      bf["full_vs_null", 1] <- bf[4,1] / bf[3,1]
      bf <- bf[c(3,4,6),]
      }
      ...
      bf <- extractBF((generalTestBF(formula = as.formula(paste0(dv, "~LSNS_base+LSNS_change+age_base+age_change+sex+education+BMI+hypertension+diabetes+CES.D+LSNS_base:LSNS_change+subject")), 
                          data=datay, whichRandom = "subject", multicore = T, 
                          neverExclude = c("age_base", "age_change", "sex", "education", "BMI", "hypertension", "diabetes", "CES.D", "subject"))),
                      logbf = F)
      bf["full_vs_null", 1] <- bf[4,1] / bf[3,1]
      bf <- bf[c(3,4,6),]
      }
  ...
  write.csv(vifres,
            file = paste0("/data/pt_life/LLammer/Results/VIFs/", dv, "_on_", poi, "_model", model, ".csv"))
  sres <- summary(res)
  coeff <- as.data.frame(sres$coefficients)
  CI <- as.data.frame(confint.merMod(res))
  coeff <- merge(coeff, CI, all = T, by="row.names")
  ...
  coeff$n_individual <- ngrps(res)
  write.csv(coeff, 
            file = paste0("/data/pt_life/LLammer/Results/coefficients/", dv, "_on_", poi, "_model", model, ".csv"))
  png(filename = paste0("/data/pt_life/LLammer/Results/diag_plots/", dv, "_on_", poi, "_model", model, ".png"))
  diagnostics.plot(res)
  dev.off()
  stab_results <- glmm.model.stab(res)
  write.csv(stab_results$summary, 
            file = paste0("/data/pt_life/LLammer/Results/model_stab/", dv, "_on_", poi, "_model", model, ".csv"))
  pval_res <- as_lmerModLmerTest(res)
...
write.csv(vif_tresh_check, file = "/data/pt_life/LLammer/Results/VIFs/threshold_check.csv")
...
write.csv(stab_warnings, file = "/data/pt_life/LLammer/Results/model_stab/warnings_check.csv")
...
write.csv(pvals, file = "/data/pt_life/LLammer/Results/pvals/overview.csv")
...
write.csv(BF, file = "/data/pt_life/LLammer/Results/bayes_factor/bayes_factors.csv")

save.image("/data/pt_life/LLammer/Results/Workspace/workspace2.RData")
