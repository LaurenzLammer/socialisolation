...
trust <- function(var){
  ...
  print(paste0("max: ", max(var, na.rm = T)))
  print(paste0("rown of max: ", which.max(var)))
  print(paste0("min: ", min(var, na.rm = T)))
...
CERADa <- replace_with_na_all(CERADa, condition = ~.x %in% c(997, 998, 999))
CERADb <- replace_with_na_all(CERADb, condition = ~.x %in% c(97, 98, 99))
fu <- Reduce(function(x,y) merge(x = x, y = y, all = T), 
             list(anam, LSNS, TMT, CERADa, CERADb, radio_fu, MRI_info_fu))
...
sem_flu_cols <- paste0("CERAD_S_", c(15, 30, 45, 60), "S_CORR")
phon_flu_cols <- paste0("CERAD_VF_", c(15, 30, 45, 60), "S_CORR")

# replace different codes with NA
df <- replace_with_na_at(df, .vars = c("TMT_TIMEA", "TMT_TIMEB", phon_flu_cols), condition = ~.x %in% c(997, 998, 999))
df <- replace_with_na_at(df, .vars = c(recognition_cols, sem_flu_cols), condition =  ~.x %in% c(97, 98, 99))
...
# the scores have to be replaced with NA if the test was marked "does not apply" (97), "unratable" (98) 
# or "refused to respond" (99) -- already done for recognition, semantic & phonemic fluency due to NA recoding 
df$learning <- ifelse(df$CERAD_WL1_97 == 1 | df$CERAD_WL1_98 == 1 | df$CERAD_WL1_99 == 1 |
                        df$CERAD_WL2_97 == 1 | df$CERAD_WL2_98 == 1 | df$CERAD_WL2_99 == 1 |
                        df$CERAD_WL3_97 == 1 | df$CERAD_WL3_98 == 1 | df$CERAD_WL3_99 == 1 , NA, df$learning)
df$recall <- ifelse(df$CERAD_WL4_97 == 1 | df$CERAD_WL4_98 == 1 | df$CERAD_WL4_99 == 1, NA, df$recall)
...
df <- df %>%
  mutate_at(.vars = c("BMI_BMI", "CESD_sum", "learning", "recall", "recognition", "sem_flu", "phon_flu", "TMT", 
              "TMT_TIMEA", "TICS_sum"), ~(scale(.) %>% as.vector)) %>%
...
df$outlier_CERAD<- ifelse(((df$fu == 0 & mean(df$cerad[df$fu == 0], na.rm = T)-df$cerad >= 2*sd(df$cerad[df$fu == 0], na.rm = T)) |
                             (df$fu == 1 & mean(df$cerad[df$fu == 1], na.rm = T)-df$cerad >= 2*sd(df$cerad[df$fu == 1], na.rm = T))),
                          1, 0)

# turn outliers (by 4SD / 3SD) of BMI / TICS into NAs
df$BMI_BMI <- ifelse(abs(df$BMI_BMI) >= 4, NA, df$BMI_BMI)
df$TICS_sum <- ifelse(abs(df$TICS_sum) >= 3, NA, df$TICS_sum)

# imputation
df$BMI_BMI <-ifelse(is.na(df$BMI_BMI), mean(df$BMI_BMI), df$BMI_BMI)
df$hypertension <- ifelse(is.na(df$hypertension), rbinom(1, 1, mean(df$hypertension, na.rm = T)), df$hypertension)
df$diabetes <- ifelse(is.na(df$diabetes), rbinom(1, 1, mean(df$diabetes, na.rm = T)), df$diabetes)
df$education <- ifelse(is.na(df$education), rbinom(1, 1, mean(df$education, na.rm = T)), df$education)
...
#write full dataset
write.csv(df, "/data/pt_life/LLammer/Data/compiled_full_data.csv")
...
df <- subset(df, is.na(df$cancer) | df$cancer == 0)#2847 -> 2648
df <- subset(df, (is.na(df$epilepsy) | df$epilepsy != 1) & (is.na(df$MS) | df$MS != 1) & 
               (is.na(df$parkinson) | df$parkinson != 1)) # 2648 -> 2614
df <- subset(df, is.na(df$centr_act_med) | df$centr_act_med != 1) # 2614-> 2388
df <- subset(df, (is.na(df$dementia) | df$dementia != 1) & (df$MMST_MMST >= 24 | (is.na(df$MMST_MMST) & 
      (is.na(df$outlier_CERAD) | df$outlier_CERAD != 1)))) # 2388 -> 2345

work <- subset(df, df$FS_usable == "qc pending")
...
length(which(bl_data$CERAD_WL1_99 == 1))
# transform preditor variables to obtain a normal distribution

df$CES.D <- log(df$CESD_sum)
...
df$CESD_sum <- ifelse(abs(df$CESD_sum-mean(df$CESD_sum, na.rm = T)) >= 4*sd(df$CESD_sum, na.rm = T), NA, df$CESD_sum)

# mark outliers (by 3SD, wave-specific) for LSNS, cognitive functions and HCV
df$outlier_LSNS <- ifelse((df$fu == 0 & diff(c(df$LSNS_sum, mean(df$LSNS_sum[df$fu == 0], na.rm = T))) >= 3*sd(df$LSNS_sum[df$fu == 0], na.rm = T) |
                             df$fu == 1 & diff(c(df$LSNS_sum, mean(df$LSNS_sum[df$fu == 1], na.rm = T))) >= 3*sd(df$LSNS_sum[df$fu == 1], na.rm = T)),
                           1, 0)
df$outlier_HCV <- ifelse((df$fu == 0 & diff(c(df$HCV, mean(df$HCV[df$fu == 0], na.rm = T))) >= 3*sd(df$HCV[df$fu == 0], na.rm = T) |
                             df$fu == 1 & diff(c(df$HCV, mean(df$HCV[df$fu == 1], na.rm = T))) >= 3*sd(df$HCV[df$fu == 1], na.rm = T)),
                          1, 0)
df$outlier_exfunct <- ifelse((df$fu == 0 & diff(c(df$exfunct, mean(df$exfunct[df$fu == 0], na.rm = T))) >= 3*sd(df$exfunct[df$fu == 0], na.rm = T) |
                            df$fu == 1 & diff(c(df$exfunct, mean(df$exfunct[df$fu == 1], na.rm = T))) >= 3*sd(df$exfunct[df$fu == 1], na.rm = T)),
                         1, 0)
df$outlier_memo <- ifelse((df$fu == 0 & diff(c(df$memo, mean(df$memo[df$fu == 0], na.rm = T))) >= 3*sd(df$memo[df$fu == 0], na.rm = T) |
                                df$fu == 1 & diff(c(df$memo, mean(df$memo[df$fu == 1], na.rm = T))) >= 3*sd(df$memo[df$fu == 1], na.rm = T)),
                             1, 0)
df$outlier_procspeed <- ifelse((df$fu == 0 & diff(c(df$procspeed, mean(df$procspeed[df$fu == 0], na.rm = T))) >= 3*sd(df$procspeed[df$fu == 0], na.rm = T) |
                             df$fu == 1 & diff(c(df$procspeed, mean(df$procspeed[df$fu == 1], na.rm = T))) >= 3*sd(df$procspeed[df$fu == 1], na.rm = T)),
                          1, 0)
