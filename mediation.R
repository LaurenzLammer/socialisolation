...
df <- read.csv("/data/pt_life/LLammer/Data/compiled_scaled_data.csv")
...
fastsem <- function(med = "HCV", dv, model = 1){
  ...
  if(max(vifres1) >= 10 | max(vifres2) >= 10){
    write_lines(paste0("VIF-threshold exceeded in mediation with ", dv, " as dependent variable 
                  ...
                  "/data/pt_life/LLammer/Results/mediation/VIF/threshold_exceeded.txt")
  }
  stest1 <- summary(test1, corr = T)
  stest2 <- summary(test2, corr = T)
  fit <- sem(formulae, df)
  sumfit <- summary(fit, fit.measures=T, rsq=T)
  write.csv(sumfit$PE, paste0("/data/pt_life/LLammer/Results/mediation/estimates/", dv, "_on_", med, "model", model, ".csv"))
  write.csv(sumfit$FIT, paste0("/data/pt_life/LLammer/Results/mediation/fit_indices/", dv, "_on_", med, "model", model, ".csv"))
...
write.csv(indirect_overview, "/data/pt_life/LLammer/Results/mediation/pvals/pvals.csv")

save.image("/data/pt_life/LLammer/Results/mediation/Workspace/workspace.RData")
